...
classDiagram
    class EffectBase["EffectBase<br>실행할 이펙트"]{
        <<abstract>>
        +Execute(EntityBase owner, SkillTableEntry entry) UniTask~bool~
...
classDiagram
    ...
    class SpawnSkillObjectEffect["SpawnSkillObjectEffect<br>스킬 오브젝트 소환"]{
	    // 액티브 스킬, 실행 효과만 작성
        +skillObject : AssetReferenceT~GameObject~ // 스킬 오브젝트
        +effects : List~EffectBase~ // 스킬 적중시 효과
    }
    
    class Projectile["Projectile<br>투사체 인스턴스 프리팹"]{
        ...
        -applyToOpponent : bool // 상대방에게 충돌 가능 여부
        -applyToFriend : bool // 아군에게 충돌 가능 여부
    }
    class SkillData["SkillData : ScriptableObject<br>스킬 효과 리스트와 기타 정보"]
    EffectBase <|-- HpUpPassiveEffect
    EffectBase <|-- DamageEffect
    EffectBase <|-- SpawnSkillObjectEffect
    Projectile --|> SkillObjectBase

    SkillObjectBase <-- SpawnSkillObjectEffect : 생성 및 이펙트 전달
...
# 임시작성
- 스킬 시스템 개편 : 스킬 데이터 편집의 유연성 증가
    - DamageEffect에 테이블 Damage에 배율을 적용시킬 수 있도록 하는 필드 추가
    - HealEffect에 테이블 Heal에 배율을 적용시킬 수 있도록 하는 필드 추가
    - SkillData
      - 스킬 시전 주체를 설정할 수 있게 하기
        - 시전자
        - 대상
      - 스킬 오브젝트의 경우 생성 위치를 설정할 수 있게 하기
        - 시전자
        - 대상
    - 스킬 시전시 참고할 컨텍스트 구조체 만들기
      - 시전자
        - 스킬을 사용한 Entity 오브젝트
      - 대상
        - 마지막으로 SkillObject등에 의해 피격당한 Entity 오브젝트