...
---
## 핵심 요소
- `SkillData` : 스크립터블 오브젝트로 정의되는 스킬의 동작 시퀀스
    - 잘 변경될 일이 없고 복잡한 시퀀스 데이터는 테이블로 작성하기 까다로우므로 스크립터블 오브젝트로 설계함
- `SkillTableEntry` : 쿨타임 공격력, 아이콘 등 수치와 텍스트 기반의 데이터
    - 기획상 변경될 일이 잦거나 테이블 기반 편집이 더 쉽고 간단한 데이터들
- `Skill` : 런타임에 생성되는 스킬 인스턴스
  - `SkillData`, `SkillTableEntry`의 정보를 참고하여 실제 스킬 실행
...
classDiagram
    class EffectBase["EffectBase<br>실행할 이펙트"]{
        <<abstract>>
        +Execute(SkillExecutionContext context, SkillTableEntry entry) UniTask~bool~ // 액티브 효과 실행
        +Activate(EntityBase target, SkillTableEntry entry) void // 패시브 효과 활성
        +Deactivate(EntityBase target, SkillTableEntry entry) void // 패시브 효과 비활성
    }
    
    class SkillData["SkillData : ScriptableObject<br>스킬 효과 시퀀스 데이터"]{
		+ IsPassiveSkill : bool // 패시브 이펙트 갯수 > 0
		+ IsActiveSkill : bool // 액티브 이펙트 갯수 > 0
        + PassiveEffects : List~EffectBase~ 
        // EntitySkillContainer에 등록시 활성화 되는 이펙트
    ...
    class EntitySkillContainer["EntitySkillContainer<br>Entity의 스킬 모음"]{
        +GetRegistedSkills() IReadonlyList~Skill~
        +RegistSkill(Skill skill) UniTask~bool~
        +UnregistSkill(Skill skill) void
    }
    ...
    class EntityBase["EntityBase:Monobehaviour<br>스킬 사용 게임오브젝트"]{
        +SkillContainer : EntitySkillContainer
    }
    
    SkillTableEntry <-- Skill : 스킬 수치 참조
    SkillData <-- Skill : 스킬 데이터 참조
    EffectBase "0..*"<--"1" SkillData
    EntitySkillContainer --> Skill  : 스킬 사용, 등록, 해제
    Skill <.. SkillFactory : SkillTableEntry기반 스킬 인스턴스 생성
    EntityBase --> EntitySkillContainer : 본인에게 패시브 스킬 효과 적용
```
- `SkillData` : 스킬의 동작 및 효과 정보, 스크립터블 오브젝트를 통해 편집
	...
	- `PassiveEffects` : 패시브 스킬 효과 리스트
		- `EntitySkillContainer`에 `RegistSkill`시 효과 실행
		- `EntitySkillContainer`에 `UnregistSkill`시 효과 제거
...
- `EntitySkillContainer` : 캐릭터의 고유 스킬들을 등록, 등록시 패시브 효과 적용
- `Skill` : 실제 스킬 인스턴스
	- 개별 스킬의 쿨다운 관리, 스킬 실행 담당
...
classDiagram
	...
	class HpUpPassiveEffect["HpUpPassiveEffect<br>패시브 체력 증가 이펙트"]{
	}
	
    class DamageEffect["DamageEffect<br>데미지 이펙트"]{
    }

    class SpawnProjectileEffect["SpawnProjectileEffect<br>투사체 소환 이펙트"]{
