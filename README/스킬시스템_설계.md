...
- `SkillTableEntry`의 레벨 관련 메서드
  - `GetLevel()` : 현재 스킬 레벨 계산 (Id - SkillRootId)
  - `CalculateNextLevelSkillId()` : 다음 레벨 스킬 ID 계산 (순수 계산만, 존재 여부 확인 안 함)
  - `CalculatePreviousLevelSkillId()` : 이전 레벨 스킬 ID 계산 (순수 계산만, 존재 여부 확인 안 함)
  - `GetSkillIdByLevel(int targetLevel)` : 특정 레벨의 스킬 ID 계산
  - 사용 예시:
    ```csharp
    // 현재 스킬의 다음 레벨 찾기
    var currentSkill = tableRepository.GetTableEntry<SkillTableEntry>(14000101); // LV1
    var nextLevelId = currentSkill.CalculateNextLevelSkillId(); // 14000102 계산
    
    // 다음 레벨이 존재하는지 확인 후 등록
    var nextEntry = tableRepository.GetTableEntry<SkillTableEntry>(nextLevelId);
    if (nextEntry != null)
    {
        await skillComponent.RegistSkill(nextLevelId); // LV2 등록
    }
    ```