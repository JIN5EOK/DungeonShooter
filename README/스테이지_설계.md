...
classDiagram
    ...
    class StageInstantiator["StageInstantiator<br>스테이지 구조 기반으로 전체 스테이지 인스턴스 생성"]{
        +InstantiateStage(...) void
    }
    
    class StageManager["StageManager<br>게임 스테이지 관리자"]{
    }
    class 팩토리들["PlayerFactory<br>EnemyFacotry<br>...<br>리소스 팩토리들"]

    class RoomCreateHelper["RoomCreateHelper<br>방 타일, 오브젝트 생성"]
    class RoomEditor["RoomEditor<br>에디터의 방 데이터 편집기"]

    RoomCreateHelper <-- StageInstantiator : 방 게임오브젝트 생성 요청
    RoomCreateHelper <-- RoomEditor : 방 게임오브젝트 생성 요청
    StageGenerator ..> IRoomDataRepository : RoomData 제공 요청
    StageManager --> StageInstantiator : 스테이지 구조 정보에 기반한 게임오브젝트 생성 요청
    ...
    StageInstantiator ..> 팩토리들 : 생성에 필요한 에셋 및 리소스 요청
```
### 스테이지 생성시 StageContext(스테이지 정보, 플레이어 정보 등) 주입구조
```mermaid
classDiagram
    class StageContext["StageContext<br>런타임 스테이지 데이터 컨텍스트"]{
        +playerConfigTableId : int
        +stageConfigTableId : int
    }
    
    class RoomDataRepository["RoomDataRepository<br>스테이지에 맞는 방 데이터 제공"]{
        -stageConfigTableEntry : StageConfigTableEntry
        +GetRandomRoomAsync(RoomType) UniTask~RoomData~
    }
    
    class EnemyFactory["EnemyFactory<br>스테이지에 맞는 적 제공"]{
        -stageConfigTableEntry : StageConfigTableEntry
        +GetRandomEnemyAsync() UniTask~Enemy~
    }

    class PlayerFactory["PlayerFactory<br>선택한 플레이어 제공"]{
        -stageContext : StageContext
        -tableRepository : ITableRepository
        +GetPlayerAsync() UniTask~Player~
    }
    
    class StageConfigTableEntry["StageConfigTableEntry : ITableEntry<br>스테이지 초기화를 위한 테이블 데이터"]{
        // 해당 스테이지에 등장할 수 있는 타일, 적, 방..
        +GroundTileKey : string
        +StageEnemiesLabel : string
        +StartRoomsLabel : string
        +NormalRoomsLabel : string
        +BossRoomsLabel : string
        ...
    }

    class PlayerConfigTableEntry["PlayerConfigTableEntry : ITableEntry<br>게임시작시 선택한 플레이어 데이터"]{
        int Id
        string Name
        string Description
        string GameObjectKey // 플레이어 게임오브젝트 어드레서블 주소
        int StartWeaponId // 시작 무기 ItemTableEntry.Id
        int Skill1Id // 1번 스킬 SkillTableEntry.Id
        int Skill2Id // 2번 스킬 SkillTableEntry.Id
    }
    SceneLoader --> StageLifeTimeScope : 씬 로드시 StageContext를 Regist 예약 요청
    class StageLifeTimeScope["StageLifeTimeScope<br>스테이지 의존성 관리자"]
    StageContext <-- PlayerFactory : 의존성 주입 받은 후 PlayerConfigTableId로 테이블 데이터 로드
    StageLifeTimeScope --> StageContext  : 컨테이너에 포함하여 다른 객체들에게 주입
    StageContext <-- RoomDataRepository : 의존성 주입 받은 후 ID로 테이블 데이터 로드
    StageContext <-- EnemyFactory : 의존성 주입 받은 후 ID로 테이블 데이터 로드
    RoomDataRepository --> StageConfigTableEntry : TableRepository에서 Id로 로드
    EnemyFactory --> StageConfigTableEntry : TableRepository에서 Id로 로드
    PlayerFactory --> PlayerConfigTableEntry : TableRepository에서 Id로 로드

```
* `StageContext`에는 선택한 플레이어 캐릭터, 플레이할 스테이지 등 런타임에 결정되는 씬 컨텍스트 정보가 담긴다
* 메인메뉴에서 플레이어가 선택한 정보를 기반으로 `StageContext`를 `SceneLoader`에게 전달
  * SceneLoader에 관해서는 [씬 설계 문서](%EC%94%AC_%EC%84%A4%EA%B3%84.md) 참고
  * 데이터 테이블에 관해서는 [데이터테이블 설계 문서](%EB%8D%B0%EC%9D%B4%ED%84%B0%ED%85%8C%EC%9D%B4%EB%B8%94_%EC%84%A4%EA%B3%84.md) 참고
...
* 타일셋·오브젝트·레이어를 표현할 때 인덱스 방식으로 표현
    * 어드레스: 주소 문자열 배열을 두고 타일/오브젝트는 해당 인덱스로 참조
    * 레이어: 레이어 이름 문자열 배열을 두고 타일 레이어는 해당 인덱스로 참조
