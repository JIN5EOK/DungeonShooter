...
```
### StageConfig과 의존성 주입을 통한 리소스 제공자 초기화 방식 구조
```mermaid
classDiagram
    class StageConfigTableEntry["StageConfigTableEntry : ITableEntry<br>스테이지 초기화 정보"]{
    ...
    class StageContext["StageContext<br>런타임 스테이지 데이터 컨텍스트"]{
        +playerPrefabKey : string
        +stageConfigTableId : string
    }
    
    사용자 --> StageContext : 스테이지 시작시 선택한 플레이어 캐릭터와 스테이지 정보 기반으로<br> StageContext 생성후 LifeTimeScope에 Regist
    ...
    StageContext <-- StageLifeTimeScope : Context 확인
    StageLifeTimeScope --> StageConfigTableEntry : Context 기반으로 Config을 찾고 의존성 컨테이너에 Regist
    StageConfigTableEntry <-- RoomDataRepository : 의존성 주입받은 정보를 기반으로 방 제공
    StageConfigTableEntry <-- EnemyFactory : 의존성 주입받은 정보를 기반으로 적 제공
    StageLifeTimeScope --> PlayerFactory   : Context 기반으로 플레이어 Key 설정
    RoomDataRepository <-- StageGenerator : 리포지토리들에서 데이터 받아 스테이지 구조 생성
    EnemyFactory <-- StageInstantiator : 팩토리들에서 데이터 받아 스테이지 오브젝트 생성
    PlayerFactory <-- StageInstantiator : 팩토리들에서 데이터 받아 스테이지 오브젝트 생성
    StageGenerator <-- StageManager : 스테이지 구조 생성 요청
    StageInstantiator <-- StageManager : 생성된 구조 기반으로 스테이지 오브젝트 생성 요청
```
