...
classDiagram
    ...
    class StageSceneInitializer["StageSceneInitializer<br>스테이지 씬 진입 시 초기화/스테이지 생성"]{
    }

    class 팩토리들["PlayerFactory<br>EnemyFacotry<br>...<br>리소스 팩토리들"]

    class RoomInstantiator["RoomInstantiator<br>방 타일, 오브젝트 생성"]
    class RoomEditor["RoomEditor<br>에디터의 방 데이터 편집기"]

    RoomInstantiator <-- StageInstantiator : 방 게임오브젝트 생성 요청
    RoomInstantiator <-- RoomEditor : 방 게임오브젝트 생성 요청
    StageGenerator ..> IRoomDataRepository : RoomData 제공 요청
    StageSceneInitializer --> StageGenerator : 스테이지 구조 생성 요청
    StageSceneInitializer --> StageInstantiator : 스테이지 구조 정보에 기반한 게임오브젝트 생성 요청
...
* 인스펙터 버튼
    ...
    * 오브젝트 배치
      * 배치할 오브젝트를 선택, 특정 키를 누르고 (Ctrl, 맥의 경우 Cmd)씬을 클릭하면 해당 위치에 오브젝트를 배치한다 
