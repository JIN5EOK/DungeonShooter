...
classDiagram
    class StageContext["StageContext<br>런타임 스테이지 데이터 컨텍스트"]{
        +playerConfigTableId : string
        +stageConfigTableId : string
    }
    ...
    class PlayerConfigTableEntry["PlayerConfigTableEntry<br>게임시작시 선택한 플레이어 데이터"]{
        int id
        string Name
        string Description
    ...
    StageContext <-- PlayerFactory : 의존성 주입 받은 후 Id로 테이블 데이터 로드
    ...
    RoomDataRepository --> StageConfigTableEntry
    EnemyFactory --> StageConfigTableEntry
    PlayerFactory --> PlayerConfigTableEntry
```
