...
classDiagram
    ...
    SceneLoader --> StageLifeTimeScope : 씬 로드시 StageContext의 Regist 예약
    class StageLifeTimeScope["StageLifeTimeScope<br>스테이지 의존성 관리자"]
    StageContext <-- PlayerFactory : 의존성 주입
    StageLifeTimeScope --> StageContext  : 컨테이너에 포함
    StageContext <-- RoomDataRepository : 의존성 주입
    StageContext <-- EnemyFactory : 의존성 주입
