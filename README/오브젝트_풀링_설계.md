...
classDiagram
    class ObjectPoolManager["ObjectPoolManager<br>오브젝트 풀 매니저"]{
        ...
        -objectFactories~int, Func < GameObject >~
        +RegistFactory(int id, Func~GameObject~ factoryLogic) void
        +Get(int id) UniTask~GameObject~
...
- `ObjectPoolManager`
    ...
    - `RegistFactory(int id, Func<GameObject>)`
        - Func는 게임오브젝트 생성 로직을 담아 넘기기 (어드레서블 기반일지 그냥 Object.Instantate기반일지 숨기기 위함)
    - `Get<GameObject>(int id)`
        - 만약 새로운 객체를 생성해야한다면 넘겼던 Func를 통해 생성
        - 풀 안에 있다면 풀 안의 객체 반환
    - `Release(int id, GameObject)`
      - 풀 안에 반환
