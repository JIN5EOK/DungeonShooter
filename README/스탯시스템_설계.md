...
classDiagram
    ...
    class EntityStats["EntityStats<br>Entity의 스탯 컨테이너"]{
        ...
        +GetStat(StatType type) EntityStat // 스탯
    }
    ...
    class EntityBase["EntityBase : Monobehaviour<br>Entity Base 컴포넌트"]{
        +StatContainer EntityStats
    }

    EntityStats ..> StatBonus : 아이템등의 스탯 보너스 수치
    StatModifierType "0..*"<--"1" EntityStat
    StatType "0..*"<--"1" EntityStats
    EntityStat "0..*"<--"1" EntityStats
    EntityStats --> EntityStatsTableEntry  : 테이블 데이터에 기반하여 기본 스탯 설정
    EntityStats <-- EntityBase : 본인의 스탯
```


- 주요 클래스와 역할
    - `EntityStatsTableEntry` : 데이터 테이블로 작성하는 각 플레이어, 적 캐릭터별 기본 스탯
    - `EntityStat` : HP, 이동속도 등의 개별 스탯 수치, 스탯 보너스를 추가할 수 있으며 추가된 스탯 보너스를 계산하여 최종 스탯 계산
