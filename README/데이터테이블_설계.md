...
classDiagram
    class ITableRepository["ITableRepository<br>테이블 데이터 제공"]{
	    +GetTableEntry~T~(int id) UniTask~T~
	}
...
* `CSVTableParser`
    ...
    * CSV 파일에서 데이터를 읽어 `SkillTableEntry`, `ItemTableEntry`로 변환
        ...
        * `SkillTableEntry.IntAmounts`, `SkillTableEntry.FloatAmounts` 의 경우 CSV파일의 해당 열에 'Json'형태의 값이 담김
          * 예: -> `{"myIntAmount":10}, {"myFloatAmount":1.0}`
          * 파싱할때 이 Json을 파싱하여 각각 IntAmounts, FloatAmounts 딕셔너리에 삽입
