...
* `ItemTableEntry` : 아이템 정보, 수치 데이터가 포함된 데이터
    * 아이템이 인벤토리에 들어오거나 사라지면 스킬컴포넌트에 스킬 등록/해제
        * UseEffect : 아이템 사용 효과
        * PassiveEffect : 아이템이 인벤토리에 들어왔을때 적용
        * EquipEffect : 무기 아이템을 장착중일때 효과
        * ActiveEffect : 무기 아이템을 사용할때 효과
* `Item` : 개별 아이템 '객체'
	* 스택 갯수 등 특정 아이템의 고유 정보 포함
* `Inventory` : 아이템 장착/소지 관리 인벤토리
	* 무기 장착시 무기 타입에 따라 장착가능 여부를 판단 (ItemType이 Weapon일 때)
