...
{
    /// <summary>
    /// 리소스를 제공자들의 기본 추상 구현 클래스, 내부 구현은 어드레서블로 하되 핸들을 반환하지 않는 구조로 어드레서블 의존성을 감춤
    /// </summary>
    public abstract class ResourceProviderBase : IResourceProvider, IDisposable
    {
        ...
        private GameObject GetInstanceInternal(AsyncOperationHandle<GameObject> handle, string address)
        {
            ...
            {
                Debug.LogError($"[{GetType().Name}] 인스턴스 생성 실패: {address}");
                return null;
            }

            if (handle.Result == null)
            {
                Debug.LogError($"[{GetType().Name}] 인스턴스가 null입니다: {address}");
                return null;
            }
        ...
        private T GetAssetInternal<T>(AsyncOperationHandle<T> handle, string address) where T : Object
        {
            if (handle.Status != AsyncOperationStatus.Succeeded)
            {
                Debug.LogError($"[{GetType().Name}] 에셋 로드 실패: {address}");
                return null;
            }
        ...
        private Sprite GetSpriteFromAtlasInternal(AsyncOperationHandle<SpriteAtlas> handle, string spriteName, string atlasAddress)
        {
            ...
            {
                Debug.LogError($"[{GetType().Name}] 스프라이트 아틀라스 로드 실패: {atlasAddress}");
                return null;
            }
            ...
            if (sprite == null)
            {
                Debug.LogError($"[{GetType().Name}] 아틀라스 내 스프라이트를 찾을 수 없음: {spriteName}");
                return null;
            }
        ...
        {
            LogHandler.Log($"[{GetType().Name}] 아틀라스 요청됨: {tag}");
