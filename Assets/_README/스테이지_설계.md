...
- private static async Task PlaceBaseTiles(Transform stageRoot, Vector2 centerPos, RoomData roomData, IStageResourceProvider stageResourceProvider)
    - 위치와 크기를 참조하여 베이스 타일 추가
- private static async Task PlaceAdditionalTiles(Transform stageRoot, RoomData roomData, IStageResourceProvider stageResourceProvider)
    - 중심위치, 위치,타일,레이어 정보를 참고하여 커스텀 타일 배치
- private static async Task<List<GameObject>>> PlaceObjectsAsync(Transform stageRoot, RoomData roomData, IStageResourceProvider stageResourceProvider)

- `RoomEditor`와 `StageInstantiator`는 RoomTilemapHelper를 통해 타일맵과 오브젝트를 생성한다

- `RoomEditor`는 `EditorStageResourceProvider`를 사용한다